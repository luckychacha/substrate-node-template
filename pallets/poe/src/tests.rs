...
fn create_claim_failed_when_claim_already_exist() {
	new_test_ext().execute_with(|| {
		let claim = vec![0; MaxLength::get() as usize];
...
#[test]
fn create_claim_failed_when_claim_over_length() {
	new_test_ext().execute_with(|| {
		// let claim = vec![0, 1, 2, 3, 4, 5];
		// better way
		let claim = vec![0; (MaxLength::get() + 1) as usize];

		assert_noop!(
			PoeModule::create_claim(Origin::signed(1), claim.clone()),
			Error::<Test>::ClaimOverLength
		);
	})
}
...
fn transfer_claim_failed_when_not_claim_owner() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			PoeModule::transfer_claim(Origin::signed(2), 1, claim.clone()),
			Error::<Test>::NotClaimOwner
		);
	})
}
